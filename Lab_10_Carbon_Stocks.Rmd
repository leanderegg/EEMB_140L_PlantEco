...
Using our estimates for above-ground biomass, we can now determine the amount of carbon stored within each survey site
...
To do this, use your DBH calculations but with the allometric equation ABG = 0.062 * dbh^2.584 and the percent carbon as 45%

```{r}

tree_biomass_oaks <- stand_data %>%
  mutate(agb_eucs = 0.009 * DBH_cm^3.638) %>% 
  mutate(agb_oaks = 0.062 * DBH_cm^2.584) %>% 
  group_by(survey_number) %>% 
  summarise(total_biomass_eucs = sum(agb_eucs), total_biomass_oaks = sum(agb_oaks)) %>% 
  mutate(plot_carbon_eucs = total_biomass_eucs * .35) %>% 
  mutate(plot_carbon_oaks = total_biomass_oaks* .45)
