...
Using the data set we uploaded above, we are going to create a bar plot
```{r}
...
There are many ways to customize graphs, such as changing colors and adding labels and legends.
...
1. Write code below to upload the NAME.csv file from Canvas. Save it as a varaible named hw_data (X point)
```{r}


```

2. Below is code to creat a bar graph using the data you loaded above. Write in words what each line of code means. Some lines have been annotated for you. (X pts)
```{r}


```

3. Copy the code above, and make alterations to add a descriptive title (1 point) and change the line color (X point)
```{r}


```

4. Create a line plot using the example data, with XX as the x variable and XX as the y variable (X points)
```{r}


```

5. Visit this website []() and add a new theme to your graph (X points)
